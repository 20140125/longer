...
class OauthController extends BaseController
{
    ...
    {
        $this->validatePost(
            ['username'=>'required|string','avatar_url'=>'required|url','email'=>'required|string|email']
        );
        ...
        if ($result) {
            ...
            if (!empty($users)) {
                ...
                $this->userModel->updateResult(objectToArray($users), 'id', $this->post['uid']);
            }
            return $this->ajaxReturn(Code::SUCCESS, 'update oauth successfully');
        }
        return $this->ajaxReturn(Code::ERROR, 'update oauth failed');
    }
    /**
     * TODO:账户绑定
     * @return JsonResponse
    ...
    public function save()
    {
        ...
        //授权账户登录没有绑定账户
        if (empty($result)) {
            return $this->ajaxReturn(Code::SUCCESS, 'successfully', ['local'=>'/admin/user/bind']);
        }
        ...
        return $this->ajaxReturn(Code::SUCCESS, 'successfully', ['oauth_url'=>$url]);
    }
    ...
    public function delete()
    {
        ...
        return $result ?  $this->ajaxReturn(Code::SUCCESS, 'remove oauth successfully')
            : $this->ajaxReturn(Code::ERROR, 'remove oauth failed');
    }
}
