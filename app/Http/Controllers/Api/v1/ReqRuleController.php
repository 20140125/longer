...
class ReqRuleController extends BaseController
{
    ...
    public function getAuth()
    {
        ...
        return $this->ajaxReturn(Code::SUCCESS, 'successfully', $ruleLists);
    }
    ...
    public function save()
    {
        $this->validatePost(
            [
                'user_id'=>'required|string', 'href'=>'required|Array',
        ...
        try {
            ...
            if (empty($users)) {
                return $this->ajaxReturn(Code::ERROR, 'user does not exist');
            }
            ...
            foreach ($this->post['href'] as $href) {
                ...
                $result = !empty($reqRes) ? $this->reqRuleModel->updateResult($req, $where) :
                    $this->reqRuleModel->addResult($req);
            }
            $message = array(
                'username' => $users->username,
                'info' => $users->username.'申请权限待审批，请周知~',
                'uid'  => $users->uuid,
                'title' => '申请权限',
                'created_at' => time()
            );
            ...
            DB::commit();
            return $result ? $this->ajaxReturn(Code::SUCCESS, 'save request authorization successfully')
                : $this->ajaxReturn(Code::ERROR, 'save request authorization failed');
        } catch (Exception $exception) {
            Log::error($exception->getMessage());
            DB::rollBack();
            return $this->ajaxReturn(Code::SERVER_ERROR, 'save request authorization failed');
        }
    }
    ...
    public function update()
    {
        if (empty($this->post['act'])) {
            ...
            return empty($result) ? $this->ajaxReturn(Code::ERROR, 'update authorization status error') :
                $this->ajaxReturn(Code::SUCCESS, 'update authorization status successfully') ;
        }
        ...
        try {
            ...
            //更新角色记录
            $this->roleModel->updateResult(
                [
                    'auth_ids'=>json_encode($role->auth_ids),
                    'auth_url'=>str_replace('\\', '', json_encode($role->auth_url))
                ],
                'id',
                $role_id
            );
            ...
            DB::commit();
            return empty($result) ? $this->ajaxReturn(Code::ERROR, 'update authorization error') :
                $this->ajaxReturn(Code::SUCCESS, 'update authorization successfully') ;
        } catch (Exception $exception) {
            Log::error(json_encode([$exception->getMessage(),$this->post]));
            DB::rollBack();
            return $this->ajaxReturn(Code::SERVER_ERROR, 'update authorization error');
        }
    }
    ...
    public function delete()
    {
        ...
        //根据用户请求授权地址获取该条规则信息
        $rule_id = $this->authModel->getResult('href', $getReq->href, '=', ['id'])->id;
        array_splice($role->auth_ids, findStrInArray($role->auth_ids, $rule_id), 1);
        ...
        DB::beginTransaction();
        try {
            $result = $this->reqRuleModel->updateResult(
                ['status'=>2,'updated_at'=>time(),'expires'=>0,'desc'=>'权限被管理员收回'],
                'id',
                $this->post['id']
            );
            if ($result) {
            ...
            DB::commit();
            return $result ? $this->ajaxReturn(Code::SUCCESS, 'remove authorization successfully') :
                $this->ajaxReturn(Code::ERROR, 'remove authorization failed');
        } catch (Exception $exception) {
            DB::rollBack();
            Log::error($exception->getMessage());
            return $this->ajaxReturn(Code::SERVER_ERROR, 'remove authorization failed');
        }
    }
}
