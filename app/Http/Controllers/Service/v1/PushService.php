...
class PushService extends BaseService
{
    ...
    public function getPushLists($form, $user, array $pagination = ['page' => 1, 'limit' => 10], array $order = ['order' => 'id', 'direction' => 'desc'], array $columns = ['*'])
    {
        ...
        $this->return['lists'] = $this->pushModel->getLists($where, $pagination, $order, $columns); //_user_lists
        foreach ($this->return['lists']['data'] as &$item) {
            $item->created_at = date('Y-m-d H:i:s', $item->created_at);
        }
        $this->return['lists']['userLists'] = Cache::get('_user_lists');
        if (empty($this->return['lists']['userLists'])) {
            $this->return['lists']['userLists'] = $this->userModel->getLists('', [], [], true, ['id', 'username','uuid']);
            Cache::put('_user_lists', $this->return['lists']['userLists'], Carbon::now()->addHours(2));
        }
        return $this->return;
    }
