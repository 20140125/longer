...
if (!function_exists('getRoundNum')) {
     ...
     * @return bool|string
     */
    function getRoundNum($length = 8, $type = 'all')
    {
        switch ($type) {
            case 'all':
...
if (!function_exists('findStrInArray')) {
    /**
     * todo:获取字符串在数组中出现的位置
     * @param $array
     * @param $str
     * @return int|string
     */
    function findStrInArray($array, $str)
    {
        $index = 0;
...
if (!function_exists('getTree')) {
     ...
     * @return array
     */
    function getTree($data, $pid, $attr = 'data', $pidAttr = 'pid')
    {
        $tree = array();
        foreach ($data as $key => $item) {
            $res = objectToArray($data[$key]);
            if ($res[$pidAttr] == $pid) {
...
if (!function_exists('formatBates')) {
     ...
     * @return string
     */
    function formatBates($size, $delimiter = '')
    {
...
if (!function_exists('getFileLists')) {
     ...
     * @return array
     */
    function getFileLists($filePath, $permissionFile = array(), $sort = 'type', $sort_order = SORT_ASC)
    {
        $fileArr = array();
        ...
        while ($file = readdir($openDir)) {
            if (!in_array($file, $permissionFile) &&
                str_replace('/', '\\', $filePath.$file) != public_path('storage')) {
                $fileArr[] = array(
                    'label' => $file,
...
if (!function_exists('unzipFile')) {
     ...
     * @return bool
     */
    function unzipFile(string $path, $resource = '', $removeResource = false)
    {
        if (!file_exists($path)) {
            return false;
        }
        ...
        $zip->close();    //关闭资源
        return $removeResource ? removeFiles($path) : true;
    }
}
...
if (!function_exists('authCode')) {
    ...
    function authCode($string, $operation = 'DECODE', $key = '', $expiry = 0)
    {
        ...
        // 密匙c用于变化生成的密文
        $keyc = $ckey_length ? ('DECODE' == $operation ? substr($string, 0, $ckey_length) :
...
if (!function_exists('base64ImageContent')) {
     ...
     * @return array|bool
     */
    function base64ImageContent($base64_image_content, string $path, string $id)
    {
...
if (!function_exists('cutFile')) {
     ...
     * @return bool
     */
    function cutFile(string $filename, string $block)
    {
        //判断是不是一个文件
...
if (!function_exists('diffTimes')) {
     ...
     * @return string
     */
    function diffTimes($s_time, $now_time, $lan = 'zh')
    {
